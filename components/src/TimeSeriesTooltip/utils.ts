...
export function getTooltipStyles(
  ...
  return {
    ...
    // LOGZ.IO CHANGE START:: Custom Drilldown preview
...
// LOGZ.IO CHANGE START:: Tooltip is not behaving correctly
            ...
            if (seriesType === 'line') {
              ...
              if (cursorPixelY !== undefined && yBufferPixels !== null) {
                ...
                if (dataPointPixel && dataPointPixel[1] !== undefined) {
                  verticalDistance = Math.abs(cursorPixelY - dataPointPixel[1]);
                  isWithinYBuffer = verticalDistance <= yBufferPixels;
                } else {
                  verticalDistance = Math.abs(visualY - cursorY);
                  isWithinYBuffer = verticalDistance <= yBuffer;
                }
              } else {
                verticalDistance = Math.abs(visualY - cursorY);
                isWithinYBuffer = verticalDistance <= yBuffer;
...
export function processCandidates(
  ...
  for (const candidate of candidates) {
    ...
    const formattedY = formatValue(valueToFormat, seriesFormat);
...
// LOGZ.IO CHANGE END:: Tooltip is not behaving correctly
